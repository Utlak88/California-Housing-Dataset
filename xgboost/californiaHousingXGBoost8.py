...
def plot_r_squared_comparison(y_test, y_predict, title):
    ...
    plt.plot(np.unique(y_test), np.poly1d(
        np.polyfit(y_test, y_predict, 1))(np.unique(y_test)))
    x_r2_label_placement = pd.Series(
        y_test).median() - 1.2 * pd.Series(y_test).std()
    y_r2_label_placement = pd.Series(
        y_predict).median() + 3 * pd.Series(y_predict).std()
    plt.text(x_r2_label_placement, y_r2_label_placement,
...
longitude, latitude, housing_median_age, total_rooms, total_bedrooms, population, households, median_income, median_house_value = range(
    0, len(data.columns))
...
normalized_columns = (columns_for_normalizing -
                      columns_for_normalizing.mean()) / columns_for_normalizing.std()
...
train_data['rooms_ratio'] = train_data['total_bedrooms'] / \
...
which_longitude_bin = np.digitize(np.reshape(train_data['longitude'].to_numpy(
), (len(train_data['longitude']), 1)), bins=bins_long)
...
which_latitude_bin = np.digitize(np.reshape(
    train_data['latitude'].to_numpy(), (len(train_data['latitude']), 1)), bins=bins_lat)
...
# Adding new feature calculating the ratio of total bedrooms to total rooms
test_data['rooms_ratio'] = test_data['total_bedrooms'] / \
...
which_longitude_bin = np.digitize(np.reshape(test_data['longitude'].to_numpy(
), (len(test_data['longitude']), 1)), bins=bins_long)
...
which_latitude_bin = np.digitize(np.reshape(
    test_data['latitude'].to_numpy(), (len(test_data['latitude']), 1)), bins=bins_lat)
...
model = xgb.XGBRegressor(objective='reg:squarederror', booster='gbtree', learning_rate=0.1,
                         n_estimators=300, eta=0.3, colsample_bynode=1, max_depth=5, alpha=1, verbosity=0)
...
print('The most important feature is {}.\n'.format(
    train_data_features.columns[model.feature_importances_.argmax()]))
################################################################################
...
plot_r_squared_comparison(test_data_target, predicted_target_feature,
                          'California Median House Value Prediction Quality\nZ-Score Normalized, Feature Added,\nFeatures Clipped, Longitude and Latitude Binned')
################################################################################
